function [reslist, convlist, exit_iter] = optimize_qN(init_struct, HistoryData, x_prev, max_iter, eps, par, method, useplot, spca, spca_method)
alpha0 = 2;
alpha = alpha0;
alpha_rate = 2;
global s1;
...
% B = inv(g_prev_p * g_prev_p');
B = eye(max(size(g_prev_p)));
g_prev_p = B * g_prev_p;
...
%             z = B * yk;
%             d = g_prev_p;
%             B = B - (z*d' + d*z')/(d'*yk) + (alpha + yk'*z/(d'*yk))*(d*d')/(d'*yk);
            % 3
%             z = B * yk;
%             d = g_prev_p;
%             B = B - alpha * d*d'/(d'*yk) - z*z'/(yk'*z);
...
%             p = plot(s1, x_new, 'color', 'b');
        ...
        alpha = min(alpha0, alpha*(alpha_rate^5));
